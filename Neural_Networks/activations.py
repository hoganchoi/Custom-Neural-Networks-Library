...
class Softmax():
    ...
    def activate(self, x): 
        ...
        '''
        x_prime = (np.exp(x)) / np.expand_dims(np.sum(np.exp(x), axis = 1), axis = 1)
        return x_prime
...
class ReLU():
    ...
    def activate(self, x):
        x_prime = np.where(x > 0, x, 0)
        return x_prime
    
    def activateGrad(self, x): 
        x_prime = np.where(x > 0, 1, 0)
        return x_prime
...
class LeakyReLU():
    ...
    def activate(self, x):
        x_prime = np.where(x > 0, x, self.alpha * x)
        return x_prime
    
    def activateGrad(self, x): 
        x_prime = np.where(x > 0, 1, self.alpha)
        return x_prime
...
class ReLU_GPU():
    ...
    def activate(self, x):
        x_prime = cp.where(x > 0, x, 0)
        return x_prime
    
    def activateGrad(self, x): 
        x_prime = cp.where(x > 0, 1, 0)
        return x_prime
...
class LeakyReLU_GPU():
    ...
    def activate(self, x):
        x_prime = cp.where(x > 0, x, self.alpha * x)
        return x_prime
    
    def activateGrad(self, x): 
        x_prime = cp.where(x > 0, 1, self.alpha)
        return x_prime
